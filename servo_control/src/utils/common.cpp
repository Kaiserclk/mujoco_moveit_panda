...
namespace servo_control
{
  ...
  /**
   * @brief 获取指定分组的逆运动学求解器基坐标系
   * @param robot_state 机器人状态
   * @param group_name 分组名称
   * @return std::optional<std::string> 逆运动学求解器的基坐标系；若未找到求解器，则返回 std::nullopt
   */
  ...
  /**
   * @brief 获取指定分组的逆运动学求解器末端执行器坐标系
   * @param robot_state 机器人状态
   * @param group_name 分组名称
   * @return std::optional<std::string> 逆运动学求解器的末端执行器坐标系；若未找到求解器，则返回 std::nullopt
   */
  ...
  /**
   * @brief 计算笛卡尔增量对应的姿态变化
   * @param delta_x 笛卡尔增量
   * @param base_to_tip_frame_transform 基坐标系到末端执行器坐标系的变换矩阵
   * @return geometry_msgs::msg::Pose 姿态变化
   */
   ...
   * @param joint_cmd_rolling_window 关节命令滚动窗口
  ...
  /**
   * @brief 更新滑动窗口
   * @param next_joint_state 下一个关节状态
   * @param joint_cmd_rolling_window 关节命令滚动窗口
   * @param max_expected_latency 最大预期延迟
   * @param cur_time 当前时间
   */
   ...
   * @param joint_state 关节状态
   * @param margins 容差
   * @return std::pair<double, StatusCode> 速度缩放因子和状态码
   */
                                                                    ...
                                                                    const servo_control::Params &servo_params)
  {
    ...
    if (is_above_lower_limit && is_below_hard_stop_limit)
    {
      ...
      servo_status = moving_towards_singularity ? StatusCode::DECELERATE_FOR_APPROACHING_SINGULARITY : StatusCode::DECELERATE_FOR_LEAVING_SINGULARITY;
    }
   ...
   * @brief 计算关节限值速度缩放因子
   * @param velocities 速度
   * @param joint_bounds 关节限值
   * @param scaling_override 缩放_override
   * @return double 缩放因子
   */
   ...
   * @param joint_bounds 关节限值
   * @param margins 容差
   * @return std::vector<size_t> 需要停止的关节变量索引
   */
  ...
  /**
   * @brief 将 Eigen::Isometry3d 转换为 geometry_msgs/TransformStamped
   * @param eigen_tf Eigen 变换矩阵
   * @param parent_frame 父坐标系
   * @param child_frame 子坐标系
   * @return geometry_msgs::msg::TransformStamped 转换结果
   */
  ...
  /**
   * @brief 从 geometry_msgs/PoseStamped 创建 PoseCommand
   * @param msg 输入的 PoseStamped 消息
   * @return PoseCommand 创建的 PoseCommand
   */
  ...
  /**
   * @brief 创建 PlanningSceneMonitor
   * @param node 节点
   * @param servo_params 伺服参数
   * @return planning_scene_monitor::PlanningSceneMonitorPtr 创建的 PlanningSceneMonitor 指针
   */
  ...
  /**
   * @brief 从 moveit::core::RobotState 提取 KinematicState
   * @param robot_state 机器人状态
   * @param move_group_name 移动组名称
   * @return KinematicState 提取的关节能状态
   */
  ...
  KinematicState extractRobotState(const moveit::core::RobotStatePtr &robot_state, const std::string &move_group_name)
  {
    ...
    // 若任意加速度值为非数值，将所有加速度置零
    // 待办：修复根本问题，消除该临时处理逻辑（工单#2958）
