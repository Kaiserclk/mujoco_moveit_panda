...
namespace servo_control
{
  ...
  Servo::Servo(const rclcpp::Node::SharedPtr &node,
               std::shared_ptr<const servo_control::ParamListener> servo_param_listener,
               const planning_scene_monitor::PlanningSceneMonitorPtr &planning_scene_monitor)
      : node_(node), logger_(rclcpp::get_logger("servo_control")), servo_param_listener_{std::move(servo_param_listener)}, planning_scene_monitor_{planning_scene_monitor}
  {
    ...
    if (!planning_scene_monitor_->getStateMonitor()->waitForCompleteState(servo_params_.move_group_name, ROBOT_STATE_WAIT_TIME))
    {
  ...
  void Servo::setSmoothingPlugin()
  {
    ...
    // 初始化平滑插件
    moveit::core::RobotStatePtr robot_state = planning_scene_monitor_->getStateMonitor()->getCurrentState();
    const int num_joints = servo_params_.joint_names.size();
  ...
  // 验证参数是否有效
  bool Servo::validateParams(const servo_control::Params &servo_params)
  {
    ...
    const auto num_dofs = servo_params.joint_names.size(); // 获取活动关节数量
  ...
  servo_control::Params &Servo::getParams()
  {
  ...
  )
  {
    ...
    if (command.index() == static_cast<size_t>(expected_type))
    {
      ...
      {
        // 处理关节空间命令
        delta_result = jointDeltaFromJointJog(std::get<JointJogCommand>(command), servo_params_);
        servo_status_ = delta_result.first;
      }
      else if (expected_type == CommandType::TWIST)
      {
        // 执行速度指令前，先将其转换至规划坐标系
        const auto planning_frame_maybe = getIKSolverBaseFrame(robot_state, servo_params_.move_group_name);
        ...
        {
          servo_status_ = StatusCode::INVALID;
          RCLCPP_ERROR(logger_, "No IK solver for planning group %s.", servo_params_.move_group_name.c_str());
        }
      }
      else if (expected_type == CommandType::POSE)
      {
        ...
        const auto planning_frame_maybe = getIKSolverBaseFrame(robot_state, servo_params_.move_group_name);
        const auto ee_frame_maybe = getIKSolverTipFrame(robot_state, servo_params_.move_group_name);
