...
constexpr int8_t KEYCODE_I = 0x69;  // Rotate X+
constexpr int8_t KEYCODE_K = 0x6B;  // Rotate X-
constexpr int8_t KEYCODE_U = 0x75;  // Rotate Y+
constexpr int8_t KEYCODE_O = 0x6F;  // Rotate Y-
constexpr int8_t KEYCODE_N = 0x6E;  // Rotate Z+
constexpr int8_t KEYCODE_M = 0x6D;  // Rotate Z-
}  // namespace
...
int KeyboardServo::keyLoop()
{
  ...
  puts("Use i|k for pitch (X), u|o for roll (Y), n|m for yaw (Z)");
  puts("SPACE to stop. 'Q' to quit.");
  ...
  for (;;)
  {
    ...
    switch (c)
    {
      ...
      case KEYCODE_SEMICOLON:
        ...
        break;
      case KEYCODE_I:
        RCLCPP_DEBUG(nh_->get_logger(), "i - pitch+");
        twist_msg->twist.angular.x = 0.5;
        publish_twist = true;
        break;
      case KEYCODE_K:
        RCLCPP_DEBUG(nh_->get_logger(), "k - pitch-");
        twist_msg->twist.angular.x = -0.5;
        publish_twist = true;
        break;
      case KEYCODE_U:
        RCLCPP_DEBUG(nh_->get_logger(), "u - roll+");
        twist_msg->twist.angular.y = 0.5;
        publish_twist = true;
        break;
      case KEYCODE_O:
        RCLCPP_DEBUG(nh_->get_logger(), "o - roll-");
        twist_msg->twist.angular.y = -0.5;
        publish_twist = true;
        break;
      case KEYCODE_N:
        RCLCPP_DEBUG(nh_->get_logger(), "n - yaw+");
        twist_msg->twist.angular.z = 0.5;
        publish_twist = true;
        break;
      case KEYCODE_M:
        RCLCPP_DEBUG(nh_->get_logger(), "m - yaw-");
        twist_msg->twist.angular.z = -0.5;
        publish_twist = true;
        break;
      case KEYCODE_1:
      ...
      case KEYCODE_SPACE:
        ...
        RCLCPP_INFO_STREAM(nh_->get_logger(), "Stopping...");
        {
          // Send zero twist
          auto stop_twist = std::make_unique<geometry_msgs::msg::TwistStamped>();
          stop_twist->header.stamp = nh_->now();
          stop_twist->header.frame_id = command_frame_id_;
          ...
          stop_joint->header.stamp = nh_->now();
